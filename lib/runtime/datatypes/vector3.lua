...
function Vector3.new(X, Y, Z)
	...
	function Proxy:Min(vector3)
		return Vector3.new(
			math.min(Properties.X, vector3.X),
			math.min(Properties.Y, vector3.Y),
			math.min(Properties.Z, vector3.Z)
		)
	end

	function Proxy:Max(vector3)
		return Vector3.new(
			math.max(Properties.X, vector3.X),
			math.max(Properties.Y, vector3.Y),
			math.max(Properties.Z, vector3.Z)
		)
	end
